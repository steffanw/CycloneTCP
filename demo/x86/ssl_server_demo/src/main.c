...
#define SERVER_DSA_CERT "..\\certs\\server_rsa_cert.pem"
#define SERVER_DSA_PRIVATE_KEY "..\\certs\\server_rsa_key.pem"
...
int_t main(void)
{
   ...
   do
   {
      ...
      error = readPemFile(SERVER_RSA_CERT, &serverDsaCert, &serverDsaCertLength);
      //Any error to report?
      if(error) break;

      //Load server's DSA private key
      error = readPemFile(SERVER_RSA_PRIVATE_KEY, &serverDsaPrivateKey, &serverDsaPrivateKeyLength);
      //Any error to report?
      if(error) break;
