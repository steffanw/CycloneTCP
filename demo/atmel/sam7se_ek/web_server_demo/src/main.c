...
int_t main(void)
{
   ...
   interface->nicDriver = &dm9000Driver;
   //Interface name
   strcpy(interface->name, "eth0");
   //Set host MAC address
   macStringToAddr("00-AB-CD-EF-75-12", &interface->macAddr);

#if (IPV6_SUPPORT == ENABLED)
   //Set link-local IPv6 address
   ipv6StringToAddr("fe80::00ab:cdef:7512", &interface->ipv6Config.linkLocalAddr);
#endif
