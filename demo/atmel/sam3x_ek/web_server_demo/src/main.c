...
int_t main(void)
{
   ...
   interface->nicDriver = &sam3xEthDriver;
   interface->phyDriver = &dm9161PhyDriver;
   //Interface name
   strcpy(interface->name, "eth0");
   //Set host MAC address
   macStringToAddr("00-AB-CD-EF-00-38", &interface->macAddr);

#if (IPV6_SUPPORT == ENABLED)
   //Set link-local IPv6 address
   ipv6StringToAddr("fe80::00ab:cdef:0038", &interface->ipv6Config.linkLocalAddr);
#endif
   ...
   //Global address
   ipv6StringToAddr("2a01:e35:8a47:b350::0038", &interface->ipv6Config.globalAddr);
   //Router
