...
int_t main(void)
{
   ...
   interface->nicDriver = &lpc18xxEthDriver;
   interface->phyDriver = &lan8720PhyDriver;
   //Interface name
   strcpy(interface->name, "eth0");
   //Set host MAC address
   macStringToAddr("00-AB-CD-EF-18-30", &interface->macAddr);

#if (IPV6_SUPPORT == ENABLED)
   //Set link-local IPv6 address
   ipv6StringToAddr("fe80::00ab:cdef:1830", &interface->ipv6Config.linkLocalAddr);
#endif
