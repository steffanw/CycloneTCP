...
//Transmit buffer
static uint8_t txBuffer[LPC18XX_TX_BUFFER_COUNT][LPC18XX_TX_BUFFER_SIZE] __attribute__((aligned(4)));
//Receive buffer
static uint8_t rxBuffer[LPC18XX_RX_BUFFER_COUNT][LPC18XX_RX_BUFFER_SIZE] __attribute__((aligned(4)));
//Transmit DMA descriptors
static Lpc18xxTxDmaDesc txDmaDesc[LPC18XX_TX_BUFFER_COUNT] __attribute__((aligned(4)));
//Receive DMA descriptors
static Lpc18xxRxDmaDesc rxDmaDesc[LPC18XX_RX_BUFFER_COUNT] __attribute__((aligned(4)));
...
void lpc18xxEthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < LPC18XX_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < LPC18XX_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ETH_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ETH_RDES1_RCH | (LPC18XX_RX_BUFFER_SIZE & ETH_RDES1_RBS1);
...
void lpc18xxEthRxEventHandler(NetInterface *interface)
{
   size_t length;
   ...
   if(LPC_ETHERNET->DMA_STAT & ETHERNET_DMA_STAT_RI_Msk)
   {
      ...
      while(1)
      {
         ...
         length = lpc18xxEthReceivePacket(interface, interface->ethFrame, ETH_MAX_FRAME_SIZE);
...
error_t lpc18xxEthSetMacFilter(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("  MAC_HASHTABLE_LOW = %08X\r\n", LPC_ETHERNET->MAC_HASHTABLE_LOW);
   TRACE_INFO("  MAC_HASHTABLE_HIGH = %08X\r\n", LPC_ETHERNET->MAC_HASHTABLE_HIGH);
   ...
   const ChunkedBuffer *buffer, size_t offset)
{
   ...
   if(length > LPC18XX_TX_BUFFER_SIZE)
   {
...
size_t lpc18xxEthReceivePacket(NetInterface *interface,
