...
//TX buffer
static uint8_t txBuffer[SAM9263_TX_BUFFER_COUNT][SAM9263_TX_BUFFER_SIZE] __attribute__((aligned(8)));
//RX buffer
static uint8_t rxBuffer[SAM9263_RX_BUFFER_COUNT][SAM9263_RX_BUFFER_SIZE] __attribute__((aligned(8)));
//TX buffer descriptors
static Sam9263TxBufferDesc txBufferDesc[SAM9263_TX_BUFFER_COUNT] __attribute__((aligned(4)));
//RX buffer descriptors
static Sam9263RxBufferDesc rxBufferDesc[SAM9263_RX_BUFFER_COUNT] __attribute__((aligned(4)));
//TX buffer index
...
error_t sam9263EthInit(NetInterface *interface)
{
   ...
   AT91C_BASE_AIC->AIC_SVR[AT91C_ID_EMAC] = (uint32_t) sam9263EthInterrupt;
...
void sam9263EthInitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < SAM9263_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < SAM9263_RX_BUFFER_COUNT; i++)
   {
...
void __irq sam9263EthInterrupt(void)
{
...
void sam9263EthRxEventHandler(NetInterface *interface)
{
   uint32_t rsr;
   uint_t length;
   ...
   const ChunkedBuffer *buffer, size_t offset)
{
   ...
   if(length > SAM9263_TX_BUFFER_SIZE)
   {
   ...
   if(txBufferIndex < (SAM9263_TX_BUFFER_COUNT - 1))
   {
   ...
   uint8_t *buffer, uint_t size)
{
   ...
   for(i = 0; i < SAM9263_RX_BUFFER_COUNT; i++)
   {
      ...
      if(index >= SAM9263_RX_BUFFER_COUNT)
         index -= SAM9263_RX_BUFFER_COUNT;
   ...
   for(i = 0; i < j; i++)
   {
      ...
      if(eofIndex != UINT_MAX && i >= sofIndex && i <= eofIndex)
      {
         ...
         n = min(size, SAM9263_RX_BUFFER_SIZE);
      ...
      rxBufferIndex++;
      //Wrap around to the beginning of the buffer if necessary
      if(rxBufferIndex >= SAM9263_RX_BUFFER_COUNT)
         rxBufferIndex = 0;;
   }
