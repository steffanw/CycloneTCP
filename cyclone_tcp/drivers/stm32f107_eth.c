...
//Transmit buffer
static uint8_t txBuffer[STM32F107_TX_BUFFER_COUNT][STM32F107_TX_BUFFER_SIZE] __attribute__((aligned(4)));
//Receive buffer
static uint8_t rxBuffer[STM32F107_RX_BUFFER_COUNT][STM32F107_RX_BUFFER_SIZE] __attribute__((aligned(4)));
//Transmit DMA descriptors
static Stm32f107TxDmaDesc txDmaDesc[STM32F107_TX_BUFFER_COUNT] __attribute__((aligned(4)));
//Receive DMA descriptors
static Stm32f107RxDmaDesc rxDmaDesc[STM32F107_RX_BUFFER_COUNT] __attribute__((aligned(4)));
...
void stm32f107EthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < STM32F107_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < STM32F107_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ETH_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ETH_RDES1_RCH | (STM32F107_RX_BUFFER_SIZE & ETH_RDES1_RBS1);
...
void stm32f107EthRxEventHandler(NetInterface *interface)
{
   ...
   if(ETH->DMASR & ETH_DMASR_RS)
   {
      ...
      while(1)
      {
         ...
         length = stm32f107EthReceivePacket(interface, interface->ethFrame, ETH_MAX_FRAME_SIZE);
...
error_t stm32f107EthSetMacFilter(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("  MACHTLR = %08X\r\n", ETH->MACHTLR);
   TRACE_INFO("  MACHTHR = %08X\r\n", ETH->MACHTHR);
   ...
   const ChunkedBuffer *buffer, size_t offset)
{
   ...
   if(length > STM32F107_TX_BUFFER_SIZE)
   {
