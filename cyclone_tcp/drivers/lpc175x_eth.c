...
#include "lpc175x_eth.h"
#include "debug.h"

//Transmit buffer
static uint8_t txBuffer[LPC175X_TX_BUFFER_COUNT][LPC175X_TX_BUFFER_SIZE] __attribute__((aligned(4)));
//Receive buffer
static uint8_t rxBuffer[LPC175X_RX_BUFFER_COUNT][LPC175X_RX_BUFFER_SIZE] __attribute__((aligned(4)));
//Transmit descriptors
static Lpc175xTxDesc txDesc[LPC175X_TX_BUFFER_COUNT] __attribute__((aligned(4)));
//Transmit status array
static Lpc175xTxStatus txStatus[LPC175X_TX_BUFFER_COUNT] __attribute__((aligned(4)));
//Receive descriptors
static Lpc175xRxDesc rxDesc[LPC175X_RX_BUFFER_COUNT] __attribute__((aligned(4)));
//Receive status array
static Lpc175xRxStatus rxStatus[LPC175X_TX_BUFFER_COUNT] __attribute__((aligned(8)));
...
void lpc175xEthInitDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < LPC175X_TX_BUFFER_COUNT; i++)
   {
   ...
   LPC_EMAC->TxDescriptorNumber = LPC175X_TX_BUFFER_COUNT - 1;
   ...
   LPC_EMAC->RxDescriptorNumber = LPC175X_RX_BUFFER_COUNT - 1;
...
void ENET_IRQHandler(void)
{
   ...
   if(status & INT_TX_DONE)
   {
      ...
      if(i >= LPC175X_TX_BUFFER_COUNT) i = 0;
...
void lpc175xEthRxEventHandler(NetInterface *interface)
{
   uint_t length;
   ...
   if(LPC_EMAC->IntStatus & INT_RX_DONE)
   {
      ...
      while(1)
      {
         ...
         length = lpc175xEthReceivePacket(interface, interface->ethFrame, ETH_MAX_FRAME_SIZE);
...
error_t lpc175xEthSetMacFilter(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("  HashFilterL = %08X\r\n", LPC_EMAC->HashFilterL);
   TRACE_INFO("  HashFilterH = %08X\r\n", LPC_EMAC->HashFilterH);
   ...
   const ChunkedBuffer *buffer, size_t offset)
{
   ...
   else if(length > LPC175X_TX_BUFFER_SIZE)
   {
      //The transmitter can accept another packet
      osEventSet(interface->nicTxEvent);
      //Report an error
   ...
   j = i + 1;
   //Wrap around if necessary
   if(j >= LPC175X_TX_BUFFER_COUNT) j = 0;
...
uint_t lpc175xEthReceivePacket(NetInterface *interface,
   uint8_t *buffer, uint_t size)
{
   ...
   //Increment index and wrap around if necessary
   if(++i >= LPC175X_RX_BUFFER_COUNT) i = 0;
   //Save the resulting value
