 ...
 * @param[out] x Resulting multiple precision integer
...
error_t tlsReadMpi(Mpi *x, const uint8_t *data, size_t size, size_t *length)
{
   error_t error;
   size_t n;
   ...
   //Convert the raw data to a multiple precision integer
   error = mpiReadRaw(x, data + 2, n);
   //Any error to report?
   if(error) return error;
...
bool_t tlsIsCertificateAcceptable(const TlsCertDesc *cert, const uint8_t *certTypes, size_t numCertTypes,
   const TlsSignHashAlgos *signHashAlgos, const TlsCertAuthorities *certAuthorities)
{
   size_t i;
   size_t n;
   size_t length;
   ...
   if(acceptable && certAuthorities != NULL)
   {
      ...
      if(length > 0)
      {
         ...
         //The list of acceptable certificate authorities decribes the known roots CA
         acceptable = FALSE;
...
error_t tlsGetCertificateType(const X509CertificateInfo *certInfo,
   TlsCertificateType *certType, TlsSignatureAlgo *certSignAlgo, TlsHashAlgo *certHashAlgo)
{
   //Check parameters
   if(certInfo == NULL || certType == NULL || certSignAlgo == NULL || certHashAlgo == NULL)
      return ERROR_INVALID_PARAMETER;
   ...
   else if(certInfo->subjectPublicKey.p && certInfo->subjectPublicKey.q &&
