...
error_t tlsSendCertificate(TlsContext *context)
{
   ...
   if(sendCert)
   {
      ...
      //Debug message
      TRACE_INFO("Sending Certificate message (%u bytes)...\r\n", length);
...
error_t tlsSendChangeCipherSpec(TlsContext *context)
{
   ...
   //Debug message
   TRACE_INFO("Sending ChangeCipherSpec message (%u bytes)...\r\n", length);
   TRACE_DEBUG_ARRAY("  ", message, length);
...
error_t tlsSendFinished(TlsContext *context)
{
   ...
   //Debug message
   TRACE_INFO("Sending Finished message (%u bytes)...\r\n", length);
   TRACE_DEBUG_ARRAY("  ", message, length);
...
error_t tlsSendAlert(TlsContext *context, uint8_t level, uint8_t description)
{
   ...
   //Debug message
   TRACE_INFO("Sending Alert message (%u bytes)...\r\n", length);
...
error_t tlsParseCertificate(TlsContext *context, const TlsCertificate *message, size_t length)
{
   ...
   //Debug message
   TRACE_INFO("Certificate message received (%u bytes)...\r\n", length);
   TRACE_DEBUG_ARRAY("  ", message, length);
   ...
   do
   {
      ...
      if(context->entity == TLS_CONNECTION_END_CLIENT)
      {
         ...
         if(context->serverName != NULL)
         {
            ...
            //If the hostnames do not match, reject the certificate
            if(i >= 0 || j >= 0)
            {
               //Debug message
...
error_t tlsParseChangeCipherSpec(TlsContext *context, const TlsChangeCipherSpec *message, size_t length)
{
   ...
   //Debug message
   TRACE_INFO("ChangeCipherSpec message received (%u bytes)...\r\n", length);
   TRACE_DEBUG_ARRAY("  ", message, length);
...
error_t tlsParseFinished(TlsContext *context, const TlsFinished *message, size_t length)
{
   ...
   //Debug message
   TRACE_INFO("Finished message received (%u bytes)...\r\n", length);
   TRACE_DEBUG_ARRAY("  ", message, length);
...
{
   //Debug message
   TRACE_INFO("Alert message received (%u bytes)...\r\n", length);
   TRACE_INFO_ARRAY("  ", message, length);
